        ...
        if div.find('WESLEY') != -1:
            ...
            for i in range(0, len(times)):
                ...
                if first_names[i] == 'Blocked':
                    ...
                    db.cur.execute("SELECT * FROM blocked_periods WHERE prac_id = 1 AND date = %s \
                                    AND start = %s AND end = %s", (date, start, end))
                    res = db.cur.fetchall()
                    if res != ():
                        continue
                    # database entry does not exist so create one
                    db.cur.execute("INSERT IGNORE INTO blocked_periods (date, start, end, prac_id) \
                                   VALUES(%s, %s, %s, %s)", (date, start, end, 1))
                    db.con.commit()
                else:
                    # check whether entry exists already
                    ...
                    db.cur.execute("INSERT IGNORE INTO clients (name, surname, dob, phone_1, \
                                    ...
                                    postcode, temp) VALUES(%s, %s, NULL, NULL, NULL, NULL, NULL, \
                                    NULL, NULL, NULL, NULL, 'Temp')", (first_names[i], surnames[i]))
                    db.con.commit()
                    ...
                    db.cur.execute("INSERT IGNORE INTO users (email, password, access_lvl, \
                                    auth_key, client_id, prac_id) VALUES('dummy', '', -1, 'dummy', \
                                    %s, NULL)", client_id)
                    db.con.commit()
                    db.cur.execute("INSERT IGNORE INTO bookings (prac_id, client_id, treat_id, \
                                    name, start, end, descr, price, pay_status) VALUES(%s, %s, %s, \
                                    %s, %s, %s, %s, %s, 'not paid')", (1, client_id, treat_ids[i], \
                                    date, start, end, notes[i], prices[i]))
                    db.con.commit()
...
#r = session_requests.get('http://rushcliff.com/r/diary.php', params=payload)
#r = session_requests.get('https://www.rushcliff.com/r/diary.php?sdate=07-03-2020', params=payload)
